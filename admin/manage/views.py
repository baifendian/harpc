...
def userLogout(request):
    ...
    """
    logout(request)
    return redirect("/manage", locals(), RequestContext(request))
...
def ajax_harpc_log(request):
    ...
    if request.method == 'POST':
        ...
        if key and type:
            h = settings.ZK_HARPC
            results = h.get_statistics(service=service, key=key, type=type, start=int(start), end=int(end))
...
def ajax_create_service(request):
    ...
    if request.method == 'POST':
        ...
        if service and len(service) > 0:
            ...
            h = settings.ZK_HARPC
            result = h.create_service(service=service)
            if result[0]:
    ...
    if request.method == 'POST':
        ...
        if service and len(service) > 0:
            if server and len(server) > 0:
                result = h.delete_server(service=service, server=server)
            else:
                result = h.delete_service(service=service)
...
def servers(request):
    ...
    if request.method == 'GET':
        ...
        h = settings.ZK_HARPC
        if service and len(service) > 0:
            isExists = h.service_exist(service=service)
            isOperate = h.get_service_acl(service=service)
    ...
    if request.method == 'POST':
        ...
        if service and len(service) > 0:
            ...
            for server in servers:
                ...
                server_info_dict = h.get_service_child_info(service=service, type=settings.ZK_SERVERS,
                                                            child=server_name)
...
def ajax_create_server(request):
    ...
    if request.method == 'POST':
        ...
        h = settings.ZK_HARPC
        result = h.create_server(service=service, server=server, server_value=server_value)
        if result[0]:
    ...
    if request.method == 'POST':
        ...
        if service and len(service) > 0:
            ...
            for client in clients:
                ...
                client_info_dict = h.get_service_child_info(service=service, type=settings.ZK_CLIENTS,
                                                            child=client_name)
...
def child(request):
    if request.method == 'GET':
        ...
        if not isExists:
            ...
            servers_total = h.get_servers_total(service)
            clients_total = h.get_clients_total(service)
